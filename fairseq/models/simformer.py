...
class SimformerEncoder(FairseqEncoder):
    """
    ...
    def __init__(self, args, dictionary, embed_tokens):
        ...
        # )
        for i in range(args.encoder_layers // 4):
            for j in range(4):
                self.layers.extend(
                    [self.build_encoder_layer(args) for j in range(4)]
                )
            self.layers.extend([FFN(args)])
    ...
    def build_encoder_layer(self, args):
        layer = SimformerEncoderLayer(args)
...
class SimformerDecoder(FairseqIncrementalDecoder):
    """
    ...
    def __init__(
        ...
        index = 0
        d = 4
        n = args.decoder_layers // d
        m = args.decoder_layers % d
        arr = []
        for i in range(n):
            for j in range(d):
                args.index = index#getattr(args, "index", index)
                if (j == d - 1):
                    args.ffn_flag = True
                else:
                    args.ffn_flag = False
    ...
    def build_decoder_layer(self, args, no_encoder_attn=False):
        print(args.ffn_flag, args.index)
        layer = SimformerDecoderLayer(args, no_encoder_attn)
        checkpoint = getattr(args, "checkpoint_activations", False)
        if checkpoint:
            offload_to_cpu = getattr(args, "offload_activations", False)
            layer = checkpoint_wrapper(layer, offload_to_cpu=offload_to_cpu)
